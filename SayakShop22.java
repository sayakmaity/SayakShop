	...
	public static void changeBrightness() {
		...
		for (int y = 0; y < newPicture.getHeight(); y++) {
			for (int x = 0; x < newPicture.getWidth(); x++) {
				...
				int ModBright = brightnessSlider.getValue();
	...
	public static void changeContrast() {
		...
		for (int y = 0; y < newPicture.getHeight(); y++) {
			for (int x = 0; x < newPicture.getWidth(); x++) {
				...
				int g = pixColor.getGreen();
				float spacer = 1;
				float ModContra = (float) contrastSlider.getValue() / 100 + spacer;
				if (ModContra < 1)
					ModContra = (float) contrastSlider.getValue() / 200 + spacer;
				r = (int) (r * ModContra);
				g = (int) (g * ModContra);
				b = (int) (b * ModContra);
				if (r > 255)
